...
ResourcesPool::Instance()
{
    ...
    return *(instance = new ResourcesPool());
}
...
Texture*
ResourcesPool::Get_texture(std::string id) const
{
...
Font*
ResourcesPool::Get_font(std::string id) const
{
    if(font_pool.find(id) != font_pool.end())
        return font_pool.at(id);
    return nullptr;
}
Sound*
ResourcesPool::Get_sound(std::string id) const
{
    if(sound_pool.find(id) != sound_pool.end())
        return sound_pool.at(id);
    return nullptr;
}
Music*
ResourcesPool::Get_music(std::string id) const
{
    if(music_pool.find(id) != music_pool.end())
        return music_pool.at(id);
    return nullptr;
}
AnimationTemplate*
ResourcesPool::Get_animation(std::string id) const
{
